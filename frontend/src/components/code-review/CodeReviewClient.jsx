...
import { getCodeReviewAnalyzeUrl, fetchCodeReviewSummary } from '@/lib/api';
// fetchRepoFileContent remains from old api
import { fetchRepoFileContent } from '@/lib/api';
import ProgressStream from '@/components/ui/ProgressStream';
import Badge from '@/components/ui/Badge';
import SeverityDistribution from '@/components/code-review/SeverityDistribution';
import GroupedIssuePanel from '@/components/code-review/GroupedIssuePanel';
...
// ── Main component ─────────────────────────────────────────────
export default function CodeReviewClient({ owner, name }) {
  ...
  const { events, status: sseStatus, error: sseError, result: rawScanResult, start } = useAnalyzeSse(getCodeReviewAnalyzeUrl);
  const starting = sseStatus === 'streaming';

  const scanPayload = rawScanResult?.data ?? null;
  const scannedFileCount = scanPayload?.scanned_files ?? files.length;

  // Transform backend result to match frontend tree structure
  useEffect(() => {
    const payload = rawScanResult?.data;
    ...
    (payload.files || []).forEach((f) => {
      ...
      (f.findings || []).forEach((finding) => {
        flattenedFindings.push({
          ...finding,
          file_path: f.path,
          finding_type: finding.type || finding.finding_type || 'unknown',
          line_start: finding.line ?? finding.line_start ?? 1,
          line_end: finding.endLine ?? finding.line_end ?? finding.line ?? 1,
        });
      });
    });
    ...
    if (flattenedFindings.length > 0) {
      ...
      setAiSummary({
        executive_summary: `Scanned **${payload.scanned_files ?? mappedFiles.length}** source files. No high-confidence issues were detected by regex or deep analysis.`,
      });
    } else {
      setAiSummary({
        executive_summary: 'No scannable source files were found. The repository may be empty, use unsupported languages, or the clone failed on the server.',
      });
    }
  }, [rawScanResult, owner, name]);
  ...
  useEffect(() => {
    ...
    setFileContent(null);
    setFileLoading(true);
    fetchRepoFileContent(owner, name, selectedFile.file_path)
      .then((res) => {
        if (!cancelled) setFileContent(res.content || '');
      })
      .catch(() => {
        if (!cancelled) setFileContent('// Could not load file content.');
      })
      .finally(() => {
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          {rawScanResult && (
            <div style={{ display: 'flex', gap: '8px' }}>
              {[
                { label: 'Files', val: scannedFileCount || files.length, color: 'var(--color-text-secondary)' },
          ...
          {!starting && (
            ...
            >
              {starting ? 'Starting…' : rawScanResult ? '↻ Re-scan' : '⚡ Run Scanner'}
            </button>
          )}
        </div>
      </div>
          ...
          >
            ...
            >
              ...
              <div style={{ padding: '4px', overflowY: 'auto', flex: 1 }}>
                {files.length === 0 ? (
                  <p style={{ padding: '12px', fontSize: '0.8125rem', color: 'var(--color-text-muted)', lineHeight: 1.5 }}>
                    No source files were indexed. Try Re-scan or check that the API can clone this repo.
                  </p>
                ) : null}
