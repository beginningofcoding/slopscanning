...
import { getDocsAnalyzeUrl, fetchRepoFileContent } from '@/lib/api';
...
function normalizeDocFinding(f) {
  const lineStart = f.line_start ?? f.line ?? (Array.isArray(f.lines) && f.lines.length ? f.lines[0] : null);
  const lineEnd = f.line_end ?? lineStart ?? (Array.isArray(f.lines) && f.lines.length ? f.lines[f.lines.length - 1] : null);
  return { ...f, line_start: lineStart, line_end: lineEnd };
}

function HighlightedMarkdown({ content, findings }) {
  // Build a set of suspicious line indices
  const suspiciousLines = new Set();
  (findings || []).map(normalizeDocFinding).forEach((f) => {
    if (f.line_start) {
...
export default function DocsReviewClient({ initialDocs = [], owner, name }) {
  const { events, status: sseStatus, error: sseError, result: docsResult, start } = useAnalyzeSse(getDocsAnalyzeUrl);
  ...
  useEffect(() => {
    ...
    async function loadContent() {
      setLoadingDoc(true);
      try {
        const res = await fetchRepoFileContent(owner, name, selectedDoc.path);
  ...
  const docFindings = (selectedDoc?.findings || []).map(normalizeDocFinding);
