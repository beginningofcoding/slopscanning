...
import { getCommitsAnalyzeUrl } from '@/lib/api';
...
const VERDICT_CONFIG = {
  ...
  TRUSTWORTHY:  { color: '#58D0A0', bg: 'rgba(88,208,160,0.08)',  border: 'rgba(88,208,160,0.25)', label: 'Trustworthy',   icon: CheckCircle2 },
  CLEAN:        { color: '#58D0A0', bg: 'rgba(88,208,160,0.08)',  border: 'rgba(88,208,160,0.25)', label: 'Trustworthy',   icon: CheckCircle2 },
};
...
/* ─── Main component ─────────────────────────────────── */
export default function CommitsReviewClient({ initialCommits = [], owner, name }) {
  const { events, status: sseStatus, error: sseError, result: commitResult, start } = useAnalyzeSse(getCommitsAnalyzeUrl);
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          {[
            ...
            { label: 'Flagged', val: displayCommits.filter(c => c.verdict && !['TRUSTWORTHY', 'CLEAN'].includes(c.verdict)).length, color: '#E8C058' },
