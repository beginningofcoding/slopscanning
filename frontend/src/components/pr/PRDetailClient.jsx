...
export default function PRDetailClient({ prDetail, owner, name }) {
  const [activeTab, setActiveTab] = useState('diff');
  const { events, status: sseStatus, error: sseError, result: reviewResult, start } = useActionStream(PR_REVIEW_ANALYZE_URL);
  ...
  return (
    <div>
      ...
      {activeTab === 'review' && reviewResult && (
        <PRReviewPanel result={reviewResult.data} />
      )}
    </div>
  );
}